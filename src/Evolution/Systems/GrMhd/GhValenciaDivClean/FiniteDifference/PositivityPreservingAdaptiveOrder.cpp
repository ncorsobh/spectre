...
void PositivityPreservingAdaptiveOrderPrim<System>::reconstruct(
  ...
  reconstruct_prims_work<tmpl::list<gr::Tags::SpacetimeMetric<DataVector, 3>>,
      ...
      // Use unlimited<2*(gzs-1)> so ghost_pts_in_neighbor_data matches
      // ghost_zone_size(). unlimited<4> assumes 3 ghost pts per stripe, but
      // neighbor_data_as_variables provides ghost_zone_size() pts per stripe;
      // mismatched strides corrupt the reconstructed metric.
