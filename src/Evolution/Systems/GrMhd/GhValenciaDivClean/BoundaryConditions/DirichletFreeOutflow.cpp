...
void DirichletFreeOutflow<System>::fd_ghost(
  ...
  *phi = get<::gh::Tags::Phi<DataVector, 3>>(boundary_values);
  ...
  const auto interior_spatial_metric =
      gr::spatial_metric(interior_spacetime_metric);
  tnsr::II<DataVector, 3, Frame::Inertial> interior_inv_spatial_metric{};
  Scalar<DataVector> det{};
  determinant_and_inverse(make_not_null(&det),
                          make_not_null(&interior_inv_spatial_metric),
                          interior_spatial_metric);
  const auto interior_shift =
      gr::shift(interior_spacetime_metric, interior_inv_spatial_metric);
  const auto interior_lapse =
      gr::lapse(interior_shift, interior_spacetime_metric);
