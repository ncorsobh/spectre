...
void fill_cell_centered_fluxes_impl(
...
/*!
 * \brief Helper function that computes cell centered fluxes from a given
 * set of primitive variables. Primarily used for filling neighbor data
 * when flux information is needed for boundaries.
 */
template <typename FluxTags, typename BoundaryVarsType>
void fill_cell_centered_fluxes(
    const gsl::not_null<Variables<FluxTags>*> flux_vars,
  ...
  detail::fill_cell_centered_fluxes_impl(
