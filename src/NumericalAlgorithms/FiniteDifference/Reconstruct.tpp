...
void reconstruct_impl(
  ...
  if constexpr (Dim > 1) {
    ...
    // Swap x and y extents so that reconstruct_impl reconstructs in y
    // (which is now the fast dimension after the above transpose)
    Index<Dim> y_first_extents = volume_extents;
    y_first_extents[0] = volume_extents[1];
    y_first_extents[1] = volume_extents[0];
    ...
    reconstruct_impl<ReturnReconstructionOrder, Reconstructor>(
        ...
        gsl::make_span(buffer.data() + volume_vars.size() + lower_ghost.size(),
                       upper_ghost.size()),
        y_first_extents, number_of_variables, args_for_reconstructor...);
    // Transpose result back: x (volume_extents[0]) was made slow, restore it
    // as fast by using it as the number_of_chunks.
    raw_transpose(
    ...
    if constexpr (Dim > 2) {
      ...
      // Swap z to be first extent so reconstruct_impl reconstructs in z
      // (which is now the fast dimension after the above transpose)
      Index<Dim> z_first_extents{};
      z_first_extents[0] = volume_extents[2];
      z_first_extents[1] = volume_extents[0];
      z_first_extents[2] = volume_extents[1];
      ...
      reconstruct_impl<ReturnReconstructionOrder, Reconstructor>(
          ...
          gsl::make_span(
              ...
              upper_ghost.size()),
          z_first_extents, number_of_variables, args_for_reconstructor...);
      // Transpose result back: chunk_size = N_x*N_y groups were made slow
      raw_transpose(
