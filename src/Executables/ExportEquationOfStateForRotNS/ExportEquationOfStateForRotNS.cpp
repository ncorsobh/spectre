...
void dump_equilibrium_eos(
  ...
  for (size_t log10_number_density_index = 0;
    ...
    const double target_log_density = log10(get_element(
        get(rest_mass_density_geometric), log10_number_density_index));
    ...
    get(pressure_geometric) =
        std::max(pow(10.0, target_log_pressure),
                 get(eos.pressure_from_density_and_temperature(
                     rest_mass_density_geometric, Scalar<double>{0.},
                     eos.equilibrium_electron_fraction_from_density_temperature(
                         rest_mass_density_geometric, Scalar<double>{0.}))));

    auto pressure_difference_from_temperature =
        [&eos, rest_mass_density_geometric,
         pressure_geometric](double temperature) {
          const Scalar<double> electron_fraction =
              eos.equilibrium_electron_fraction_from_density_temperature(
                  rest_mass_density_geometric, Scalar<double>{temperature});
          const Scalar<double> pressure =
              eos.pressure_from_density_and_temperature(
                  rest_mass_density_geometric, Scalar<double>{temperature},
                  electron_fraction);
          return get(pressure) - get(pressure_geometric);
        };

    const Scalar<double> temperature =
        Scalar<double>{RootFinder::toms748<false>(
            pressure_difference_from_temperature, 0., 1.,
