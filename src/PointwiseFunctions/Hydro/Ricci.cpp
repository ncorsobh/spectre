...
void ricci_tensor(const gsl::not_null<tnsr::aa<DataType, 3>*> result,
...
tnsr::aa<DataType, 3> ricci_tensor(
  ...
  ricci_tensor(make_not_null(&result), stress_energy, spacetime_metric);
  return result;
}
}  // namespace hydro
...
#define INSTANTIATE(_, data)                                 \
  template void hydro::ricci_tensor(                         \
  ...
  template tnsr::aa<DTYPE(data), 3> hydro::ricci_tensor(     \
      const tnsr::AA<DTYPE(data), 3>& stress_energy,         \
      const tnsr::aa<DTYPE(data), 3>& spacetime_metric);
