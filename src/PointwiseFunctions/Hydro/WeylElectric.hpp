 ...
 * as the spatial components of \f$ E_{ab} = E_{ab}^{(\mathrm{vac})} -
 * \frac{1}{2}(\gamma_a^c\gamma_b^d + \gamma_{ab}\gamma^{cd}){}^{(4)}R_{cd} +
 * \frac{1}{3}\gamma_{ab}{}^{(4)}R\f$, where \f$E_{ab}^{(\mathrm{vac})}\f$ is
 * the vacuum contribution to the electric part of the Weyl tensor (computed in
 * `GeneralRelativity`), \f$\gamma_{ab}\f$ is the induced spatial metric,
 * \f${}^{(4)}R_{ab}\f$ is the Ricci tensor, and \f${}^{(4)}R\f$ is the Ricci
 * scalar.
 */
...
struct WeylElectricCompute : WeylElectric<DataType, 3>, db::ComputeTag {
  using argument_tags =
      tmpl::list<gr::Tags::WeylElectric<DataType, 3>,
                 ...
                 hydro::Tags::Ricci<DataType, 3>,
                 hydro::Tags::RicciScalar<DataType>,
  ...
  using base = WeylElectric<DataType, 3, Frame::Inertial>;
};
...
struct WeylElectricScalarCompute : WeylElectricScalar<DataType>,
                                   db::ComputeTag {
  using argument_tags =
      tmpl::list<hydro::Tags::WeylElectric<DataType, 3, Frame::Inertial>,
