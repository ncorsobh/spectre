...
void test_fd(const U& boundary_condition, const T& analytic_solution_or_data) {
  ...
  const typename grmhd::ValenciaDivClean::System::variables_tag::type
      ghost_cons_vars{subcell_mesh.number_of_grid_points()};
  ...
  boundary_condition.fd_ghost(
      ...
      get<gr::Tags::SpacetimeMetric<DataVector, 3>>(prim_vars),
